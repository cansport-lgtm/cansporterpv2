...
export default function ConsumptionEfficiencyDashboard() {
  ...
  const { data: categoryBreakdown } = useQuery({
    ...
    queryFn: async () => {
      ...
      data?.forEach((item: any) => {
        ...
        grouped[category].total += Number(item.actual_consumption) || 0;
      });
  ...
  const totalStandard = varianceData?.reduce((sum, v) => sum + v.standard, 0) || 0;
  const totalActual = varianceData?.reduce((sum, v) => sum + v.actual, 0) || 0;
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <MetricCard
            ...
            value={bomCount || 0}
            icon={BarChart3}
            description="Consumption recipes"
          />
          <MetricCard
            title="Today's Consumption"
            value={`${totalActualConsumption.toFixed(2)} kg`}
            icon={TrendingDown}
            description="Actual material used"
          />
          <MetricCard
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Standard Consumption (7 Days)</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Actual Consumption (7 Days)</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className={`text-2xl font-bold ${overallVariancePct > 5 ? "text-destructive" : overallVariancePct < -5 ? "text-green-600" : ""}`}>
                {overallVariancePct > 0 ? "+" : ""}{overallVariancePct.toFixed(1)}%
              </div>
              <p className="text-xs text-muted-foreground">
                {overallVariancePct > 0 ? "Over consumption" : overallVariancePct < 0 ? "Under consumption" : "On target"}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="variance" className="space-y-4">
          <TabsList>
            <TabsTrigger value="variance">Variance Analysis (7 Days)</TabsTrigger>
          ...
          <TabsContent value="category">
            <Card>
              <CardHeader>
                <CardTitle>Category-wise Consumption ({format(weekStart, "MMM d")} - {format(weekEnd, "MMM d, yyyy")})</CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
