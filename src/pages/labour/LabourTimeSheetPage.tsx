...
const LabourTimeSheetPage = () => {
  ...
  const { data: employees = [] } = useQuery({
    queryKey: ["labour-employees-timesheet"],
    queryFn: async () => {
  ...
  const { data: publicHolidays = [] } = useQuery({
    queryKey: ["labour-public-holidays-timesheet", format(currentMonth, "yyyy")],
    queryFn: async () => {
  ...
  const { data: entries = [] } = useQuery({
    queryKey: ["labour-timesheet-entries", format(currentMonth, "yyyy-MM")],
    queryFn: async () => {
    ...
    if (viewMode === "daily") {
      ...
      const tableRows = filteredEmployees.map((employee) => {
        ...
        const dayCells = daysInMonth.map((day) => {
          ...
          const attendance = attendanceMap[employee.id]?.[dateKey];
          if (attendance) {
            const cls = attendance.totalMph >= 12 ? 'present' : 'half-day';
            const code = attendance.totalMph >= 12 ? 'P' : 'H';
            return `<td class="${cls}">${code}<div style="font-size:7px;opacity:.75">${attendance.checkIn || '-'}</div><div style="font-size:7px;opacity:.75">${attendance.checkOut || '-'}</div></td>`;
          }
      ...
      printWindow.document.write(`
        <!DOCTYPE html><html><head>
          <title>Time Sheet - ${format(currentMonth, "MMMM yyyy")}</title>
          ${styles}
  ...
  const getAttendanceCell = (employeeId: string, date: Date) => {
    ...
    const cellClass = cn(
      "w-[78px] h-[58px] flex flex-col items-center justify-center gap-0.5 rounded px-1",
      isToday && "ring-2 ring-primary",
    );
    const badgeClass = "px-1.5 py-0 h-4 text-[10px] leading-none font-semibold";
    const timeClass = "font-mono text-[9px] leading-tight opacity-80";
    ...
    const renderCell = (
      ...
      <div className={cn(cellClass, bgClass)} title={title}>
        <Badge variant="secondary" className={cn(badgeClass, bgClass)}>{label}</Badge>
        <span className={timeClass}>{inTime}</span>
        <span className={timeClass}>{outTime}</span>
      </div>
    );

    if (isFuture) {
      return renderCell("bg-muted/30 text-muted-foreground", "-", "-", "-");
    }
    if (isPublicHoliday(dateKey)) {
      return renderCell(
        ...
        "-",
        "-",
        `Public Holiday: ${publicHolidayMap[dateKey]}`,
      );
    }
    if (isSunday(date)) {
      return renderCell("bg-gray-500/20 text-gray-500", "S", "-", "-", "Sunday");
    }
    ...
    if (attendance) {
      ...
      return renderCell(
        ...
        attendance.checkIn || "-",
        attendance.checkOut || "-",
        `${attendance.workType} (${attendance.totalMph} MPH)`,
      );
    }
  ...
  return (
    <ERPLayout>
      <PageHeader
        title="Time Sheet"
        description="Monthly & daily check-in / check-out times based on productivity entries"
                        ...
                        {daysInMonth.map((day) => {
                          ...
                          return (
                            <th
                              key={day.toISOString()}
                              className={cn(
                                "px-1 py-3 text-center text-xs font-medium min-w-[80px]",
...
export default LabourTimeSheetPage;
