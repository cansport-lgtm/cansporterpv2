-- ============================================================
-- Periodic COGS RPC — final version used by /accounting/periodic-cogs
-- Aggregates inventory closings + RM purchases server-side in one round-trip.
-- Avoids Supabase's 1000-row client cap.
-- ============================================================
...
DECLARE
  ...
  v_purch    NUMERIC := 0; v_prod    NUMERIC := 0; v_already NUMERIC := 0;
...
BEGIN
  ...
  SELECT COALESCE(SUM(total_amount), 0) INTO v_already
  FROM public.accounting_vouchers
  ...
  production_additions := v_prod; already_posted_cogs := v_already;
