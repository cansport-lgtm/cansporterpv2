...
BEGIN
  ...
  IF m.from_location_id IS NOT NULL
    ...
    -- Get last balance from ledger for this item+location
    SELECT COALESCE(l.balance_quantity, 0)
    INTO last_balance
    FROM public.floor_inventory_ledger l
    WHERE l.item_name = m.item_name AND l.location_id = loc
    ...
    WHERE s.item_name = m.item_name AND s.location_id = loc
    LIMIT 1;
    ...
    ELSE
      ...
      SET quantity = new_qty, unit = effective_unit
